									 ...
									 fix_windowagg_cond_context *context)
{
	...
	if (IsA(node, WindowFunc))
	{
		...
		/*
		 * The expression should be in the target list, but if it's part of a
		 * QUALIFY clause that references a WindowFunc that's not in the
		 * original target list, the node will be marked as resjunk and won't
		 * be found.
		 */
		if (newvar)
			return (Node *) newvar;
